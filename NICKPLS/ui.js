        ...
        <div id="infoSidebar" class="nb-sidebar">
            ...
            <div class="nb-sidebar-header">
                <div style="display: flex; align-items: center; justify-content: space-between; width: 100%;">
                    <h2 class="nb-title" style="margin: 0;">NICKPLS</h2>
                    <div id="refreshLobbyBtn" class="nb-refresh-btn" title="Force Refresh Lobby" onclick="window.refreshLobby()">
                        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M21.5 2v6h-6M2.5 22v-6h6M2 11.5a10 10 0 0 1 18.8-4.3M22 12.5a10 10 0 0 1-18.8 4.3"/>
                        </svg>
                    </div>
                </div>
                <div class="nb-copy-all-btn" onclick="window.copyAllNicks(this)" style="margin-top: 10px;">COPY ALL NICKS</div>
...
export function populateContent(displayData, linkHTML, matchData, lobby, ranks, summoners, premadeGroups = []) {
    ...
    displayData.forEach((_, index) => {
        ...
        let activeMatches = getSoloQMatches(m);
        let dataSource = "SoloQ Matches";

        if (!activeMatches || !activeMatches.winList) {
            activeMatches = m; // Fallback to all matches if no soloQ played recently
            dataSource = "All Matches";
        }

        const win = activeMatches?.winList ? calculateWinRate(activeMatches.winList) : "N/A";
        const role = m?.laneList ? mostCommonRole(m.laneList) : "N/A";
        const kda = activeMatches ? calculateKDA(activeMatches.killList, activeMatches.assistsList, activeMatches.deathsList).replace(" KDA", "") : "N/A";
        ...
        card.innerHTML = `
            ...
            <div class="nb-card-stats">
                <div class="nb-stat-block" title="Source: ${dataSource}">
                    <div class="nb-stat-value">${win}</div>
                    <div class="nb-stat-label">WINRATE</div>
                </div>
                <div class="nb-stat-block" title="Source: All Matches">
...
function renderProfileHeader(playerData) {
    ...
    let solo = getSoloQMatches(matches);
    ...
    header.innerHTML = `
        ...
        <div class="nb-profile-stats-grid">
            <div class="nb-stat-item" title="Source: ${solo === matches ? 'All Matches' : 'SoloQ Matches'}">
            ...
            <div class="nb-stat-divider"></div>
            <div class="nb-stat-item top-champs" title="Source: All Matches">
